...
object Day20 extends Day(20, 2021) {
  val algo = input.head.zipWithIndex.filter(_._1 == '#').map(_._2).toSet

  case class Image(
      litInBounds: Set[(Int, Int)],
      bounds: ((Int, Int), (Int, Int)),
      outOfBoundsLit: Boolean
  ) {
    ...
    def enhance: Image = {
      val seq = for {
        ...
        y <- (minY - 1) to (maxY + 1)
        b = Integer.parseInt(
          square(x, y).map(isLit).map(b => if (b) '1' else '0').mkString,
          2
        )
        if algo(b)
      } yield (x, y)
  ...
  val initialImage = {
    val set = input
      ...
      .zipWithIndex
      .flatMap { case (line, x) =>
        line.zipWithIndex.collect { case ('#', y) =>
          x -> y
        }
      }
      .toSet
    Image(
      set,
      (
        (set.map(_._1).min, set.map(_._2).min),
        (set.map(_._1).max, set.map(_._2).max)
      ),
      false
    )
  }
  ...
  def loop(n: Int): String =
    LazyList
      .iterate(initialImage)(_.enhance)
      .drop(n)
      .head
      .litInBounds
      .size
