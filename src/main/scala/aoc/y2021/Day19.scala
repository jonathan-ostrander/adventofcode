...
object Day19 extends Day(19, 2021) {
  implicit class Beacon(val point: (Int, Int, Int)) {
    ...
    def rotations: List[Beacon] =
      List(
        ...
        .flatMap { case (i, j, k) =>
          List((i, j, k), (j, -i, k), (-i, -j, k), (-j, i, k))
        }

    /** Move this point to a different scanner's relative system
      *
      * @param toMoveTo
      *   Root beacon to move to relative to root scanner
      * @param equivalent
      *   Equivalent beacon in this beacon's scanner's system
      * @return
      *   Relative position of this beacon to the root scanner
      */
  ...
  val scanners: List[Scanner] =
    input
      .filter(_.nonEmpty)
      .foldLeft(List.empty[Scanner]) { case (acc, line) =>
  ...
  val (merged, points) =
    loop(scanners.head.beacons.toSet, List((0, 0, 0)), scanners.tail)
