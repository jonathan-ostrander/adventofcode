...
object Day18 extends Day(18, 2021) {
  sealed trait Tree {
    def magnitude: Long
    ...
    def explode: Option[Tree] = {
      def loop(stack: List[Branch], visited: Set[Branch]): Option[Tree] =
        stack match {
          case Nil => None
          case head :: tail =>
    ...
    def parse(string: String): Tree = {
      def loop(stringLeft: String): (Tree, String) =
        if (stringLeft.head.isDigit) {
          Leaf(stringLeft.takeWhile(_.isDigit).toInt) -> stringLeft.dropWhile(
            _.isDigit
          )
        } else {
