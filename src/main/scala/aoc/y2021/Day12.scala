...
object Day12 extends Day(12, 2021) {
  ...
  case class Path(
    ...
    def nextPaths: List[Path] = {
      val (newVisited, newCanRevisit, newDidRevisit) =
        ...
        else if (didRevisit.exists(_ == current))
  ...
  def solve(paths: List[Path], completed: Int): Int = paths match {
    ...
    case path :: tail =>
      if (
        path.isComplete && path.canRevisit.isEmpty && path.didRevisit.isEmpty
      ) {
  ...
  override def partTwo(): String = {
    val allTheSmallThings =
      graph.keys
        .filterNot(k => k.head.isUpper || k == "start" || k == "end")
        .map(Some.apply)
        .toList
    solve(
      (None :: allTheSmallThings).map(canRevisit =>
        Path(Set.empty, "start", canRevisit, None)
      ),
      0
    ).toString
  }
